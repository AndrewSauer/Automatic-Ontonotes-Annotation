...
#Note: due to this clunky solution, if these values are changed in onf2tensor, they must be changed here as well
BATCH_SIZE=1
GLOBAL_SIZE=120
NUM_ENTITIES=90
...
class EtcModel(tf.keras.layers.Layer):
  ...
  def __init__(self,
    ...
    #final_layer_model=tf.keras.models.Sequential();
    #final_layer_model.add(tf.keras.Input(shape=(BATCH_SIZE,NUM_ENTITIES,NUM_ENTITIES,config.hidden_size)));
    #final_layer_model.add(tf.keras.layers.Dense(units=98));#96 relations, identity/coref, and no relation
    #final_layer_model.compile();
    self.final_layer=tf.keras.layers.Dense(units=98)#96 relations, identity/coref, and NR/no relation
  ...
  def call(self,
    ...
    local_output,global_output=self.global_local_transformer(
        ...
        training=training)
    #CONTINUE: Feed concatenated embedding pairs through the final layer
    #Also we need to figure out how to get the batch_size and global_seq_len, why isn't it in the model config?
    #Check this code, it seems shaky
    entities=global_output[:,GLOBAL_SIZE-NUM_ENTITIES:GLOBAL_SIZE,:]#Currently, this is 30:120
    all_pairs_shape=tf.stack([tf.shape(entities)[0],tf.shape(entities)[1],tf.shape(entities)[1],tf.shape(entities)[2]],axis=0)
    all_pairs_shape=tf.convert_to_tensor([BATCH_SIZE,NUM_ENTITIES,NUM_ENTITIES,self.config.hidden_size],tf.int32)
